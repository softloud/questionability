...
subtitle: "Why and how <br> I use a modern data stack for small and  enterprise-scale projects alike"
author: "Interruption warmly encouraged! <br> github: softloud/questionability"
date: "August 6, 2026"
format:
  revealjs:
    theme: solarized
...
142 expert teams, same data, same questions
...
```{r}
tit_exp <- readr::read_csv("../source-data/tit_experiment.csv")

dim_tit <- dim(tit_exp)

```

data: `r dim_tit[1]` rows x `r dim_tit[2]` columns
| teams: 77
...
```{r}
euc_exp <- readr::read_csv("../source-data/euc_experiment.csv")
dim_euc <- dim(euc_exp)     
```

data: `r dim_euc[1]` rows x `r dim_euc[2]` columns
| teams: 66
:::
...
featured with **blue tits** and **eucalyptus** background
...
```{r}
model_hist <- readr::read_rds("../figures/rds/model-counts--histogram.rds")
...
```{r}
conclusions_treemap <- readr::read_rds("../figures/rds/conclusions--treemap.rds")
...
```{r}
conclusive_qualified_treemap <- readr::read_rds("../figures/rds/conclusions--treemap--conclusive-qualified.rds")

conclusive_qualified_treemap
...
```{r alluvial}
conclusions_alluvial <- readr::read_rds("../figures/rds/evidence-conclusion--alluvial.rds")

conclusions_alluvial
...
Orchestrators enable conducting instrumentalists playing different instruments to produce a concerto of analytic outputs with the domain expert as the soloist.
...
- **domain soloist**: Ecologist (R) 

- **orchestral accompaniment**: Datapunk (jill of all languages, mistress of none) -> no domain knowledge 
...
::: {.callout-tip title="The essence of orchestration"}
...
## let's run it and break it!

```shell
run dagster dev -m dagster_qn

```

---

![](dagster-pipeline-fail-collapsed.svg){width="100%"}
